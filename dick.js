...
function resizeElement(target)
{
	...
	var eW = target.clientWidth;
	var eH = target.clientHeight;

	if (eW == 0)
	{
		//console.log('elementHidden');
		eW = parseInt(target.style.width);
		eH = parseInt(target.style.height);
	}
	...
	var adjust = amountZoom;

	//console.log('old dimensions: '+eH+', '+eW+'. element resize called, '+action+' performed, adjusted by '+adjust);
...
function scrollFunction()
{
	...
	var BGWidthString = document.getElementById('BGWidthString').value;
	var BGWidthArray = BGWidthString.split(',');
	var BGHeightString = document.getElementById('BGHeightString').value;
	var BGHeightArray = BGHeightString.split(',');
	var scrollWrapper = document.getElementById('scrollWrapper');
	var scrollWidth = scrollWrapper.scrollLeft;
	var browserWidth = window.innerWidth;
	var tiles = document.getElementsByClassName('dicktile');
	...
	for (var i = 1; i <= BGNum; i++)
	{
		if (scrollWidth > (i * BGWidth) - drawAheadWidth)
		{
			if (!document.getElementById('bg' + i))
			{
				...
				currentBGImg.style.width = BGWidthArray[i];
				currentBGImg.style.height = BGHeightArray[i];
				currentBGImg.className = 'extraBackgroundImage';
				resizeElement(currentBGImg)
...
function loadMove()
{
	...
	var dickSlideIn = setInterval(function()
	{
		...
		if (Math.abs(currentPos) < (150 * speed))
		{
			if (!accelCalced)
			{
				var vF = 0;
